...
def apply_sync_prod(sync_prod, initial_marking, final_marking, cost_function, skip, ret_tuple_as_trans_desc=False,
    ...
    return __search(sync_prod, initial_marking, final_marking, cost_function, skip, {None: 0}, incidence_matrix,
                    ret_tuple_as_trans_desc=ret_tuple_as_trans_desc, max_align_time_trace=max_align_time_trace)


def __search(sync_net, ini, fin, cost_function, skip, split_lst, incidence_matrix, ret_tuple_as_trans_desc=False,
             max_align_time_trace=sys.maxsize):
    start_time = time.time()
    ...
    # b_matrix = np.asmatrix(incidence_matrix.b_matrix).astype(np.float64)
    ...
    ini_state = SearchTuple(0 + h, 0, h, ini, None, None, x, True, None)
    open_set = [ini_state]
    ...
    traversed = 0
    lp_solved = 1
    max_events = max(split_lst.values())
    ...
    while not len(open_set) == 0:
        ...
        while not curr.trust:
            if split_point not in split_lst and split_point not in temp_split:
                split_lst[split_point] = max_events
                print(split_point, split_lst)
                ...
                if trustable != 'Optimal':
                    print("remove split:", split_lst)
                    temp_split[split_point] = 1
                    del split_lst[split_point]
                else:
                    print(t_index, x)
                    return __search(sync_net, ini, fin, cost_function, skip, split_lst, incidence_matrix, ret_tuple_as_trans_desc=False,\
                                max_align_time_trace=sys.maxsize)
            ...
            tp = SearchTuple(curr.g + h, curr.g, h, curr.m, curr.p, curr.t, x, True, curr.last_sync)
        ...
        # if len(split_lst)> 3:
        #     print("\ncurr cost", curr.g, curr.t, "This is the lst:",trans_to_visit_with_cost)
        ...
        for t, cost in trans_to_visit_with_cost:
            ...
            if new_marking not in dict_g:
                ...
                if t.label[0] == t.label[1]:
                    last_sync = t
                    tp = SearchTuple(new_f, g, h, new_marking, curr, t, x, trustable, last_sync)
                else:
                    tp = SearchTuple(new_f, g, h, new_marking, curr, t, x, trustable, curr.last_sync)
            else:
                if curr.g + cost < dict_g[new_marking]:
                    ...
                    for i in open_set:
                        if i.m == new_marking:
                            ...
                            i.trustable = utils.__trust_solution(x)
                            new_f = g + h
                            i.t = t
                            i.p = curr
                            print("find", i.t, get_pre_events(i))
                            if t.label[0] == t.label[1]:
                                last_sync = t
                                tp = SearchTuple(new_f, g, h, new_marking, curr, t, x, trustable, last_sync)
                            else:
...
class SearchTuple:
    def __init__(self, f, g, h, m, p, t, x, trust, last_sync):
        self.f = f
        self.g = g
        self.h = h
        self.m = m
...
def get_max_events(marking):
    ...
    if marking.t.label[0] == marking.t.label[1]:
        return get_max_events(marking.p) + 1
    else:
        return get_max_events(marking.p)

def get_pre_events(marking):
    if marking.t == None:
        return
    print(marking.t.label)
    return get_pre_events(marking.p)
