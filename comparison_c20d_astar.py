...
event_log = xes_importer.apply('C:\data\log_c.xes')
model_net, model_im, model_fm = import_net('C:\data\model_c.pnml')
for case_index, case in enumerate(event_log):
    ...
    try:
        ...
        # print(align)
        align = astar.apply(case, model_net, model_im, model_fm, violate_lst, trace_lst)
        align['time'] = time.time() - start_time
        # align['time'] = time.time() - start_time
        # print(case_index, align['restart'])
        with open('C:\data\c20c_astar_latest.csv', 'a') as f_object:
