...
def search():
    ...
    event_log = xes_importer.apply('log\RTF_s1.xes')
    model_net, model_im, model_fm = import_net('model\Rtf02.pnml')
    ...
    df.to_csv('F:\Thesis\data\data_traffic_fine\RTF_tue_open_set_heapq_s1.csv', sep=',', index=False)
    ...
    for case_index in tqdm(range(len(event_log))):
        ...
        with open('F:\Thesis\data\data_traffic_fine\RTF_tue_open_set_heapq_s1.csv', 'a') as f_object:
    ...
    df = pd.read_csv('F:\Thesis\data\data_traffic_fine\RTF_tue_open_set_heapq_s1.csv')
    total = df.sum()
    ...
    df3.to_csv('F:\Thesis\data\data_traffic_fine\RTF_tue_open_set_heapq_s1.csv', index=False)
