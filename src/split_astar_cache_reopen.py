...
class AlignmentWithCacheReopenAstar:
    """
    ...
    def search(self):
        ...
        while True:
            ...
            while self.open_set.lst:
                ...
                self.queue += timeit.default_timer() - start_time
                # if do not activate infeasible set, then compare f-value to the minimum in infeasible.
            ...
            if len(self.infeasible_set) > 0:
                ...
                if self.max_rank + 2 < self.trace_len and self.max_rank + 2 not in self.split_lst:
                    ...
                    if feasible_flag:
                        ...
                        self.queue += timeit.default_timer() - start_time
                        self.split_open_set_flag = True
                    # if some markings in cache set become feasible, then put feasible markings to open set, infeasible markings
                    # to infeasible set
                    else:
                        self.max_rank = last_max_rank
    ...
    def _update_marking(self, marking, new_sol, h):
        ...
        trust_flag = False
        # use h_to_add_lst to store all possible h-value
        ...
        for t, cost in trans_to_visit_with_cost:
            ...
            if new_m in closed:
                ...
                if marking_to_explore.g > new_g:
                    ...
                    new_h, new_x, trustable = self._derive_heuristic(cost_vec, curr.x, t_idx, curr.h)
                ...
                # put all markings into open set, regardless of feasiblity of markings
                else:
                    if m_in_open_flag:
                        start_time = timeit.default_timer()
                        marking_to_explore = self.open_set.heap_get(new_m)
                        ...
                        # subsequent marking has equal path, but the heuristic change from infeasible to feasible
                            ...
                            if update_flag:
                                ...
                                marking_to_explore.parikh_vec_lst = new_parikh_vec_lst
                                new_h, new_x, trust_flag = self._derive_heuristic(cost_vec, curr.x, t_idx, curr.h)
    ...
    def _derive_heuristic(self, cost_vec, solution_lst, t_idx, h):
