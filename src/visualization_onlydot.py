...
def gviz_state_change(ts, curr_state, feasiblee_state_lst, infeasiblee_state_lst, visited, split_lst, open_set):
    for state in ts.states:
        state.label = state.name
    filename = tempfile.NamedTemporaryFile(suffix='.gv')
    viz = Digraph(ts.name, filename=filename.name, engine='dot', graph_attr={'bgcolor': 'transparent'})
    ...
    for s in ts.states:
        ...
        elif s.name in feasiblee_state_lst:
            viz.node(str(id(s)), str(""), style='filled', fillcolor="#1E90FF")
        elif s.name in infeasiblee_state_lst:
            viz.node(str(id(s)), str(""), style='filled', fillcolor="#FF0000")
        else:
            pass
    for t in ts.transitions:
        if t.from_state.name in infeasiblee_state_lst and (t.to_state.name in feasiblee_state_lst or t.to_state.name in infeasiblee_state_lst):
            viz.edge(str(id(t.from_state)), str(id(t.to_state)), label="")
        elif t.from_state.name in feasiblee_state_lst and (t.to_state.name in feasiblee_state_lst or t.to_state.name in infeasiblee_state_lst):
            viz.edge(str(id(t.from_state)), str(id(t.to_state)), label="")
        elif t.from_state.name in curr_state and (t.to_state.name in feasiblee_state_lst or t.to_state.name in infeasiblee_state_lst):
            viz.edge(str(id(t.from_state)), str(id(t.to_state)), label="")
        else:
            pass
    viz.attr(overlap='false')
...
def viz_state_change(ts, curr_state, feasiblee_state_lst, infeasiblee_state_lst, visited, split_lst, open_set):
    for state in ts.states:
        state.label = state.name
    filename = tempfile.NamedTemporaryFile(suffix='.gv')
    viz = Digraph(ts.name, filename=filename.name, engine='dot', graph_attr={'bgcolor': 'transparent'})
