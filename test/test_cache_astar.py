...
from src.split_astar_cache import AlignmentWithCacheAstar
...
def compute_alignment(xes_file, pnml_file):
    ...
    # get the name of event log and model
    ...
    for case_index in tqdm(range(len(event_log))):
        ...
        if trace_str not in trace_variant_lst:
            ...
            # compute alignment with split-point-based algorithm + caching strategy
            start_time = timeit.default_timer()
            ali_with_split_astar = AlignmentWithCacheAstar(initial_marking, final_marking, cost_function,
    ...
    df.to_csv('../results/log=' + log_name + '&model=' + model_name + '&algorithm=cache_astar' + '.csv', index=False)
