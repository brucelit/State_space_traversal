...
from src.split_astar_cache_reopen_counter import AlignmentWithCounterAstar
from src.tools import SynchronousProduct


def compute_alignment(xes_file, pnml_file, counter=1):
    """
    ...
    # the column name in result file
    field_names = ['case_id',
                   'total',
                   'heuristic',
                   'queue',
    ...
    for case_index in tqdm(range(len(event_log))):
        ...
        if trace_str not in trace_variant_lst:
            ...
            # compute alignment with split-point-based algorithm + counter settings
            start_time = timeit.default_timer()
            ali_with_split_astar = AlignmentWithCounterAstar(initial_marking, final_marking, cost_function,
                                                           incidence_matrix, trace_sync, trace_log, counter)
    ...
    # The name of result csv file is of the form: 'log_name + model_name + algorithm type + counter value.csv'
    df.to_csv('../results/log=' + log_name + '&model=' + model_name + '&algorithm=counter' + str(counter) + '_astar' + '.csv', index=False)
