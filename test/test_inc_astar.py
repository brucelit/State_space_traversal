...
from src.split_astar import AlignmentWithSplitAstar
...
def compute_alignment(xes_file, pnml_file):
    ...
    for case_index in tqdm(range(len(event_log))):
        ...
        if trace_str not in trace_variant_lst:
            ...
            # compute alignment with split-point-based algorithm
            start_time = timeit.default_timer()
            ali_with_split_astar = AlignmentWithSplitAstar(initial_marking, final_marking, cost_function,
    ...
    df.to_csv('../results/log=' + log_name + '&model=' + model_name + '&algorithm=inc_astar' + '.csv', index=False)
