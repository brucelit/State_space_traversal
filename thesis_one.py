...
def astar(sync_net, sync_im, sync_fm, cost_function):
    ...
    h_score = {}
    g_score = {}
    a_matrix = np.asmatrix(incidence_matrix).astype(np.float64)
    print("a_matrix",incidence_matrix)
    print("p_index",place_index)
    print("t_index",trans_index)
    enabled_trans = []
    for trans in trans_index:
        enabled_trans.append(trans)
    h_score[ini_tuple], solution_x = utilities.compute_estimated_heuristic(ini_vec, fin_vec, a_matrix, cost_vec)
    g_score[ini_tuple] = 0
    ...
    print("consumption_matrix", consumption_matrix)
    ...
    print("t_index", trans_index)
    print("p_index", place_index)
    print("consumption matrix", consumption_matrix)
    ...
    while len(open_set) > 0:
        ...
        new_lst = sorted(enabled_trans, key=lambda k: k.label)
        for t in new_lst:
            traversed += 1
