...
def astar_with_split(order, ts, place_map, sync_net, sync_im, sync_fm, consumption_matrix, incidence_matrix, p_index, t_index, cost_function, split_lst, visited):
    """
    ------------
    # Line 1-10
    ------------
    ...
    while len(open_set) > 0:
        ...
        if curr.marking_tuple in heuristic_set:
            ...
            if new_split_point not in split_lst:
                ...
                return astar_with_split(order, ts, place_map, sync_net, sync_im, sync_fm, consumption_matrix, incidence_matrix, p_index, t_index, cost_function, split_lst, visited)
        ...
        for t in enabled_trans:
            ...
            if new_tuple not in dict_g:
                dict_g[new_tuple] = 100000

            if new_tuple not in closed_set:
                traversed += 1
                # create new state
                # print("\n current marking",curr.marking)
                # print("\n current solution vector", curr.parikh_vector)
                not_trust, new_heuristic, new_parikh_vector, a, t, new_pre_trans_lst, last_sync = compute_new_state_test(curr, cost_vec, t, t_index )
                ...
                if a <= dict_g[new_tuple]:
                    ...
                    if not_trust == 1:
                        ...
                        f = g + new_heuristic
                        h = new_heuristic
                    new_state = util.State(not_trust, f, g, h, new_marking, new_tuple, curr, t, new_pre_trans_lst, last_sync, new_parikh_vector)
                    queued += 1
                    # compute cost so far
                ...
                for i in open_set:
                    if i.marking_tuple == new_tuple:
                        if f <= i.f and new_state.not_trust == 0:
